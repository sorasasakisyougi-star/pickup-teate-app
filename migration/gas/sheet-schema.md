...
`区分=バス` の行は、既存本線 (`pages/api/powerautomate.ts:1023-1024`) と同じく
`出発地` (F) と `到着1..8` (G..N) を **空欄** で保存する。
金額 (Q) は一律 2000、総走行距離 (R) は 0。ODO始 / ODO終 (O/P) と 備考 (S) は通常どおり書く。

例:
| 報告時刻 (A) | 日付 (B) | 運転者 (C) | 車両 (D) | 区分 (E) | 出発地 (F) | 到着1 (G) | ... | 到着8 (N) | ODO始 (O) | ODO終 (P) | 金額 (Q) | 総走行距離 (R) | 備考 (S) |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| `2026-04-19T10:15:00+09:00` | `2026/4/19` | 山田太郎 | ハイエース | バス | (空) | (空) | ... | (空) | 215159 | 215185 | 2000 | 0 | (任意) |
